...
public class SerialLogic implements SerialInterface {
    ...
    public void serialConnection(boolean connected) {
        if(connected) {
            ...
            mConnectDialog.dispose();
        } else {
            Log.logMsg("Serial could not connect.");
            
            MigLayout layout = new MigLayout();
            
            mConnectDialog = UIManager.getSingleton().createDialog();
            mConnectDialog.setLayout(layout);
            
            Panel pan = new Panel(new MigLayout());
            mConnectDialog.add(pan);
            pan.setBorder(BorderFactory.createLineBorder(Color.black));
            
            pan.add(UIManager.getSingleton().createLabel("SoundFun could not detect a SoundBox."), "wrap");
            pan.add(UIManager.getSingleton().createLabel("Please connect your SoundBox to continue..."));
            
            // Start a blocking dialog prompting user to connect serial device.
            UIManager.getSingleton().getMainWindow().addDialog(
                    mConnectDialog,
                    true
                    );
        }
